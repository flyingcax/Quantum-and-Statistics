...
- 谐振子体系的哈密顿量： $H = \sum_j \frac{1}{2m_j}\hat{p}_j^2 + \frac{1}{2}m_j \omega_j^2 \hat{x}_j^2$
...
- 哈密顿量： $H = \sum_j \omega_j(\hat{p}_j^2 +\hat{x}_j^2)/2$
- 产生-湮灭算符： $\hat{a}=(\hat{x}+i\hat{p})/\sqrt{2}\,,\,\hat{a}^{\dagger}=(\hat{x}-i\hat{p})/\sqrt{2}$
- 哈密顿量： $H =\sum_j \omega_j (\hat{a}_j^\dagger\hat{a}_j + \frac{1}{2})=\sum_j \omega_j$
